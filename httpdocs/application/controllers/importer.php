...
";
    ...
    foreach ($citibikejsonsmap as $citibikejson) {
      ...
      echo $i.'/'.$tot. ' ' .count($json['stations']). ' : '.$parseTime.' : Memory: '. memory_get_usage() .'<br><script>toBottom(); document.title="'.$i.'/'.$tot.'"</script>';
      ...
      if(!is_array($json) || empty($json['executionTime']) || !is_array($json['stations']) ) {
    ...
    $count = $this->db->query('SELECT COUNT(*) as count FROM stations')->row()->count;
    echo '<h1>'.$count.'</h1>';
  ...
  private function _checkfiles($name) {
    if(!is_array($this->_parsed)) {
      $query = $this->db->distinct()->select('parseTime')->from('stations')->get();
